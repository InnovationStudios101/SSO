...
if (_player_class in _unit_class_array) then {
	while {local player} do {
		...
		if (gp_sleepingbag_act_id_deploy_sleepingbag == 9999) then {
			if (!gp_sleepingbag_deployed) then {
				gp_sleepingbag_act_id_deploy_sleepingbag = player addAction ["Deploy SleepingBag", "core\scripts\gp_sleepingbag\deploy_sleepingbag.sqf", "", 0, false, true, "", ""];
			};
		};
		// remove action to deploy...
		if (gp_sleepingbag_act_id_deploy_sleepingbag != 9999) then {
			if (gp_sleepingbag_deployed) then {
				player removeAction gp_sleepingbag_act_id_deploy_sleepingbag;
