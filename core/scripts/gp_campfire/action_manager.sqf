...
if (_player_class in _unit_class_array) then {
	while {local player} do {
		// add action to light
		if (gp_campfire_act_id_create_campfire == 9999) then {
			if (!gp_campfire_created) then {
				gp_campfire_act_id_create_campfire = player addAction ["Make Campfire", "core\scripts\gp_campfire\light_fire.sqf", "", 0, false, true, "", ""];
			};
		};
		...
		if (gp_campfire_act_id_create_campfire != 9999) then
		{
			if (gp_campfire_created) then
			{
				player removeAction gp_campfire_act_id_create_campfire;
